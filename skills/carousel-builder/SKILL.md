...
When generating a BSL carousel, **Claude picks the `type` for each content slide based on what the source material best fits**:
- **paragraph** — one prose idea that breathes. Use when the point is conceptual and benefits from a few full sentences.
- **bullets** — 2 to 4 short parallel items that share a structure ("three signs of...", "key drivers", "what changes").
- **checklist** — 2 to 4 short action items the reader could tick off ("steps to take", "do today", "before the audit").
...
7. **Content slide bodies** depend on the slide type. **Keep everything concise — short, scannable lines. Don't pad paragraphs with hedges or recap.**
   - **No big paragraph blocks.** Any multi-sentence text (cover/outro descriptions and paragraph bodies) is automatically reflowed by the script: one sentence per line, with a blank line after every two sentences. So no block of text ever runs longer than two sentences before a break. You don't need to insert the line breaks yourself — just write clean sentences and keep the total short. Aim for at most 4 sentences total in a paragraph body (two blocks of two).
   - **paragraph** (v2 BSL only): `body` = a tight set of **2 to 4 sentences** that breathe vertically (the script blocks them in pairs). If it would run longer than 4 sentences, split it across two paragraph slides or trim. Never use a paragraph slide as a dumping ground.
