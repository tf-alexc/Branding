#!/usr/bin/env python3
"""
Generate a course dates PDF from a sub-brand template using PyMuPDF.

Template has two pages:
  Page 0 — two-column layout  → used when column_type == "both"
  Page 1 — single-column layout → used for all other column types

Strategy (page 0 only):
  1. Strip separator Form XObject calls (Fm7/8/9/10) from content streams
     BEFORE apply_redactions — after that call streams are merged into compact
     single-line format and the multi-line regex no longer matches.
  2. Pass 1 apply_redactions: remove placeholder text, LINE_ART_NONE keeps all
     remaining Form XObjects and inline shapes intact.
  3. Insert new text on top.

Page 1 is already single-column; no XObject stripping required.
"""

import fitz, os, re

TEMPLATE_DIR = "/Users/alexcraiu/Desktop/Claude Playground/Course PDFs"
...
def fill_course(brand, course_name, left_label, left_dates,
    ...
    output_dir    = os.path.join(OUTPUT_BASE, f"Course - {brand}")
    os.makedirs(output_dir, exist_ok=True)
    output_path   = os.path.join(output_dir, f"Course - {brand} - {_safe_name(course_name)}.pdf")
    ...
    if column_type == "both":
        ...
        # Step 1: strip separator XObjects BEFORE apply_redactions
        _strip_xobjs(doc, page, _SEP_XOBJS)

        # Step 2: redact placeholder text (LINE_ART_NONE preserves graphics)
        ...
        # Step 1: redact placeholder text (page 1 is already single-column,
        # no XObject stripping needed — its content stream is already compact)
        page.add_redact_annot(_TITLE_REDACT,      fill=())
...
if __name__ == "__main__":
    fill_course(
        ...
        course_name="UK CAA / EASA Part 21 Subpart J - Understanding & Optimising your Design Organisation Approval (DOA)",
        left_label="Upcoming Dates",
        left_dates=["02 Jul - 03 Jul 2026", "14 Sep - 15 Sep 2026", "23 Nov - 24 Nov 2026"],
        column_type="generic",
    )
